...
cleaned_and_enriched as (
    select
        -- Generate unique trip identifier using PACKAGE (surrogate key pattern)
        {{ dbt_utils.generate_surrogate_key(['u.vendor_id', 
                                            'u.pickup_datetime', 
                                            'u.pickup_location_id', 
                                            'u.service_type']) }} as trip_id,
            -- formerly
                -- concat (u.vendor_id, u.pickup_datetime, u.pickup_location_id, u.service_type) as trip_id
